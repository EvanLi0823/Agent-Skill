---
name: skill-creator
description: Guide for creating effective skills and agents, adding skill references, skill scripts or optimizing existing skills/agents. This skill should be used when users want to create a new skill or agent (or update an existing one) that extends Claude's capabilities. All created skills and agents MUST include a prohibition module (NEVER/DO NOT with reasons) per references/prohibition-principles.md.
...
Every Agent file in `.claude/agents/` MUST start with YAML frontmatter or it will NOT be recognized by Claude Code:

```yaml
---
name: unity-developer          # kebab-case, matches filename (without .md)
description: |                 # 1024 chars max — critical: determines WHEN Claude invokes this agent
  Use this agent when... Include <example> blocks with user/assistant/commentary.
tools: Glob, Grep, Read, Edit, Write, Bash, TodoWrite  # comma-separated
model: sonnet                  # optional: sonnet / haiku / opus
color: green                   # optional: red/orange/yellow/green/cyan/purple
---
```

Available tools: `Glob`, `Grep`, `Read`, `Edit`, `Write`, `Bash`, `TodoWrite`, `WebFetch`, `WebSearch`, `BashOutput`, `KillShell`

**REQUIRED: Prohibition Module**

Every Skill and Agent MUST include a prohibition module following the patterns in `references/prohibition-principles.md`. Add it at the top of the content body (after frontmatter):

- **Skill**: add `## 本技能禁令 (NEVER / DO NOT)` section with NEVER/DO NOT items, each with an `原因：` line
- **Agent**: add `## 角色禁令 (NEVER / DO NOT)` after the role description, plus a `## 沟通规范` section
- Each prohibition must target the real, high-frequency mistakes for that specific skill/agent
- Workflow-type skills must also include a "懒惰借口反驳" table

Refer to `references/prohibition-principles.md` for templates, 14-principle mapping, and the quality checklist.

To complete SKILL.md, answer the following questions:

1. What is the purpose of the skill, in a few sentences?
2. When should the skill be used?
3. What prohibitions apply? (See `references/prohibition-principles.md`)
4. In practice, how should Claude use the skill? All reusable skill contents developed above should be referenced so that Claude knows how to use them.
...
## References
- [Prohibition Principles](references/prohibition-principles.md) — 禁令优先原则模板和 14 条原则速查（创建 Skill/Agent 必读）
- [Skill TDD 方法论](references/skill-tdd.md) — Iron Law、RED-GREEN-REFACTOR 循环、CSO description 规则、测试借口反驳表（发布前验证必读）
- [Anthropic Best Practices](references/anthropic-best-practices.md) — 官方 Skill 创作最佳实践（精简、结构、测试）
